...
//            __unsafe_unretained MJPhoto *photo = _photo;
            //modify by logic 2016年4月1日
            __weak MJPhotoView *photoView = self;
            __weak MJPhoto *photo = _photo;
            [_imageView sy_setImageWithURL:_photo.url placeholderImage:_photo.placeholder completed:^(UIImage *image, NSError *error, NSURL *imageURL) {
                photo.image = image;
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        [_imageView sy_setImageWithURL:_photo.url placeholderImage:_photo.placeholder progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            if (loading && receivedSize > kMinProgress) {
...
- (void)adjustFrame
{
    ...
    CGSize imageSize = _photo.image ?  _photo.image.size : _imageView.image.size;
    CGFloat imageWidth = imageSize.width > 0.0 ? imageSize.width : 1.0;
    CGFloat imageHeight = imageSize.height > 0.0 ? imageSize.height : 1.0;
    ...
    if (widthScale < heightScale) {
        ...
        imageFrame = CGRectMake(0, 0, imageWidth * boundsHeight / imageHeight, boundsHeight);
...
- (void)saveToAlbum:(UILongPressGestureRecognizer *)gesture
{
                ...
                IBActionSheet *choiceSheet = [[IBActionSheet alloc] initWithTitle:nil
                                                                         callback:^(IBActionSheet *actionSheet, NSInteger buttonIndex) {
                                                                             if (buttonIndex == 0) {
                                                                                 ...
                                                                                 [library saveImage:self.photo.image toAlbum:APPNAME completion:^(NSURL *assetURL, NSError *error) {
                                                                                     if (error) {
                                                                                         [SYUtils showWindowLevelNotice:@"图片保存失败"];
                                                                                     } else {
                                                                                         [SYUtils showWindowLevelNotice:@"保存图片成功"];
                                                                                     }
...
{
    // 取消请求
    [_imageView sy_setImageWithURL:[NSURL URLWithString:@"file:///abc"]];
}
@end