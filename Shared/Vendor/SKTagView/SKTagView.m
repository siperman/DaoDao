...
//          去除空的 “全部” 标签
            ...
            if (previewsView)
            {
                ...
                {
                    if (lineCount + 1 > self.maxLines) {
                        self.isConsealed = YES;
...
- (void)onLongPress:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if (self.didLongPressTagAtIndex) {
            self.didLongPressTagAtIndex([self.subviews indexOfObject:gesture.view]);
        }
    }
}
