...
{
    int index = 0;
    for (MGSwipeButton * button in buttons) {
...
-(void) transitionStatic:(CGFloat) t
{
    ...
    for (NSInteger i = buttons.count - 1; i >=0 ; --i) {
        MGSwipeButton * button = [buttons objectAtIndex:i];
...
-(void) layoutSubviews
{
    ...
    imgSep.frame = CGRectMake(0, self.bounds.size.height - 2, self.bounds.size.width, 1);
}
...
-(void) createSwipeViewIfNeeded
{
    if (!swipeOverlay) {
        ...
        if (!imgSep) {
            ...
            imgSep.frame = CGRectMake(0, swipeOverlay.bounds.size.height - 1, swipeOverlay.bounds.size.width, 1);
        }
        [swipeOverlay addSubview:imgSep];
